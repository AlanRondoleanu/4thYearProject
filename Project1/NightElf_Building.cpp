...
NightElf_Building::NightElf_Building(const sf::Vector2f& position, bool isEnemy)
	: Buildings(position, isEnemy)
{
	...
	// Texture
	loadTileTexture("Assets/Textures/Fantasy/NightElf_Building.png");
...
void NightElf_Building::update(float t_deltaTime)
{
	if (getAlive())
	{
		// Health Set
...
std::string NightElf_Building::GetUnitType() const
{
	return "NightElf_Building";
}
