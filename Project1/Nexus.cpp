...
Nexus::Nexus(const sf::Vector2f& position, bool isEnemy)
	: Buildings(position, isEnemy)
{
	// Texture
	loadTileTexture("Assets/Textures/Sci-fi/Nexus.png");
...
void Nexus::update(float t_deltaTime)
{
	if (getAlive())
	{
		// Health Set
...
std::string Nexus::GetUnitType() const
{
	return "Nexus";
}
