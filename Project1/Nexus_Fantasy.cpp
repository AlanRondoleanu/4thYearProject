...
Nexus_Fantasy::Nexus_Fantasy(const sf::Vector2f& position, bool isEnemy)
	: Buildings(position, isEnemy)
{
	// Texture
	loadTileTexture("Assets/Textures/Fantasy/Nexus.png");
...
void Nexus_Fantasy::update(float t_deltaTime)
{
	if (getAlive())
	{
		// Health Set
...
std::string Nexus_Fantasy::GetUnitType() const
{
	return "Nexus_Fantasy";
}
