...
Turret_Fantasy::Turret_Fantasy(const sf::Vector2f& position, bool isEnemy)
	: Buildings(position, isEnemy)
{
	// Texture
	loadTileTexture("Assets/Textures/Fantasy/Turret.png");
...
void Turret_Fantasy::update(float t_deltaTime)
{
	if (getAlive())
	{
		// Health Set
...
std::string Turret_Fantasy::GetUnitType() const
{
	return "Turret_Fantasy";
}
